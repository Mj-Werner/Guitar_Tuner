...
void loop() {
  ...
  while (cycleCount < NUM_CYCLES) {
    ...
    filteredVal = filteredVal + alpha * (raw - filteredVal); // IIR low-pass filter
    int val = (int)filteredVal;
      ...
      if (now - lastAcceptedPeakTime >= IGNORE_TIME) {
        if (lastPeakTime > 0) {
          ...
          if (period >= PERIOD_MIN && period <= PERIOD_MAX) {
            // Reject outliers if inconsistent with previous periods
            if (cycleCount > 0) {
              sortArray(periods, cycleCount);
              unsigned long recentMedian = periods[cycleCount / 2];
              if (abs((long)period - (long)recentMedian) > recentMedian * 0.2) {
                lastPeakTime = now;
                lastAcceptedPeakTime = now;
                lastVal = val;
                continue; // Skip inconsistent period
              }
            }
